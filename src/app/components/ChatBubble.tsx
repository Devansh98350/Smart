...
export default function ChatBubble({ key, content, role }: ChatBubbleProps) {
  ...
  const messageContainerClass = isUser ? 'userMessageContainer' : 'aiMessageContainer';
  const contentClass = isUser ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300';

  return (
    <div key={key} className={`flex items-start w-fit max-w-[410px] mb-[-2] ${alignmentClass}`}>
      ...
      <div className={`flex items-center justify-${isUser ? 'end' : 'start'} space-x-1 rtl:space-x-reverse ${messageContainerClass}`}>
        <div className={`flex flex-col leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700 ${textAlignClass}`}>
